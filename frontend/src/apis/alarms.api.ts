...
export const createAlarm = (payload: CreateAlarmPayload) => {
  return api.post<Alarm>(`${BASE_URL}/alarms`, payload)
}

export const getGroupAlarms = (groupId: string) => {
  return api.get<Alarm[]>(`${BASE_URL}/alarms/group/${groupId}`)
}

export const getUserAlarms = () => {
  return api.get<Alarm[]>(`${BASE_URL}/alarms`)
}

export const deleteAlarm = (alarmId: string) => {
  return api.delete(`${BASE_URL}/alarms/${alarmId}`)
}