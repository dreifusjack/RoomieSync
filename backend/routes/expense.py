...
def ExpensesRoutes(app: Flask, supabase: Client):
    ...
    def calculate_expenses(group_id):
        ...
        for i in range(len(users)):
            for j in range(i + 1, len(users)):
                user1 = users[i]
                user2 = users[j]
                amount_user1_owes_user2 = per_person_share[user2] - per_person_share[user1]
                amount_user2_owes_user1 = per_person_share[user1] - per_person_share[user2]
        ...
        return jsonify({'net_owes': net_owes}), response.status_code
    ...
    def get_expense_by_id(id):
        response = supabase.table('expenses').select('*').eq('id', id).execute()
        return jsonify(response.data), response.status_code
